...
from app.sdk.cache import Cache, fresh
...
class SubscribeCal(_PluginBase):
    ...
    def search_sub(self, sub: Subscribe, cache: bool = True) -> Optional[List[CalendarInfo]]:
        """搜索订阅"""
        tmdbid = self.get_tmdb_id(sub)
        if tmdbid is None:
            logger.warning(f"{sub.name} ({sub.year}) 不是有效的 TMDB 订阅，跳过日历处理")
            return None
        cache_key = self.get_sub_key(sub)
        if cache_key is None:
            return None
        cache_params = {"key": cache_key, "region": self._search_sub_region}
        info = None
        if cache and result_cache and result_cache.exists(**cache_params):
            ...
            with fresh(not cache):
                if sub.episode_group and (result := TmdbApi().get_tv_group_detail(group_id=sub.episode_group, season=sub.season)):
                    ...
                    result = self.chain.tmdb_info(tmdbid=tmdbid, mtype=MediaType(sub.type), season=sub.season)
            if result:
    ...
    def media_process(self, sub: Subscribe, cal_info: list[CalendarInfo]) -> Optional[str]:
        ...
        if _key is None:
            logger.warning(f"{title_year} 不是有效的 TMDB 订阅，跳过日历事件处理")
            return None
        # 剧集播出时间
        minutes = (
    ...
    def generate_average_time(self, sub: Subscribe, cal_info: list[CalendarInfo]) -> Optional[float]:
        """
        计算剧集播放时间
        ...
        tmdbid = self.get_tmdb_id(sub)
        if tmdbid is None:
            logger.info(f"{sub.name} ({sub.year}) 不是有效的 TMDB 订阅，无法计算播放时间")
            return None
        if histories := [his for his in DownloadHistoryOper().get_last_by(
            mtype=sub.type,
            title=sub.name,
            year=sub.year,
            ...
            media_id=str(tmdbid),
        ) if verify_downloadhis_note(his.note)]:
            # 初始化分集更新时间
    ...
    @staticmethod
    def get_tmdb_id(sub: Subscribe) -> Optional[int]:
        """读取 TMDB 订阅的来源原生 ID，拒绝其他媒体来源。"""
        media_source, media_id = resolve_media_identity(media=sub)
    ...
    @classmethod
    def get_sub_key(cls, sub: Subscribe) -> Optional[str]:
        """构造 TMDB 订阅的日历缓存键。"""
        tmdbid = cls.get_tmdb_id(sub)
        if tmdbid is None:
            return None
        return f"__key_{tmdbid}_{sub.year}_{sub.season}__"
